...
import { 
  Upload, CheckCircle, AlertTriangle, Cpu, LogOut, LayoutDashboard, History, 
  BarChart3, Shield, Zap, Database, Search, Bell, RefreshCw, ExternalLink, 
  Download, Camera, Sun, Moon, Layers, ZapOff, Activity, ArrowRight
} from 'lucide-react';
...
function App() {
  ...
  const handleLoginSuccess = (res) => {
    ...
    setUser(decoded);
    localStorage.setItem('texScanUser', JSON.stringify(decoded)); // Save login session
    setView('app');
  };
  ...
  const uploadImage = async () => {
    if (!file || isAdmin) return; // Admin cannot scan
    setLoading(true);
  ...
  // Filtered history for Admin Search
    ...
    return (
      <div style={{...styles.landingWrapper, background: darkMode ? '#020617' : '#ffffff'}}>
        ...
        <header style={styles.heroSection}>
          <h1 style={{...styles.heroTitle, color: darkMode ? '#fff' : '#020617'}}>AI Fabric <span className="gradient-text">Inspector</span></h1>
    ...
    <div style={{ display: 'flex', minHeight: '100vh', backgroundColor: darkMode ? '#020617' : '#f8fafc', color: darkMode ? '#f1f5f9' : '#1e293b' }}>
      ...
      <nav style={{...styles.sidebar, background: darkMode ? '#0f172a' : '#1e40af'}}>
        ...
        <div style={styles.profileSection}>
          ...
          <div style={{overflow:'hidden'}}><p style={{margin:0, fontSize:'0.8rem', fontWeight:'bold'}}>{user.name.split(' ')[0]}</p></div>
        ...
        <header style={styles.mainHeader}>
          ...
          <div style={styles.headerActions}>
            <div style={styles.searchBar}>
              <Search size={18} color="#64748b"/><input placeholder="Search users or files..." style={styles.searchInp} onChange={(e) => setSearchTerm(e.target.value)}/>
            </div>
        ...
        {activeTab === 'dashboard' ? (
          <div style={styles.dashboardGrid}>
            <div style={styles.statsRow}>
              <StatBox label="Total Scans" val={stats.total} icon={<Layers color="#3b82f6"/>} trend="Live"/>
              <StatBox label="Pass Rate" val={`${(stats.passed/stats.total*100 || 0).toFixed(1)}%`} icon={<CheckCircle color="#10b981"/>} trend="Global"/>
              <StatBox label="Avg Accuracy" val={`${stats.avgAccuracy}%`} icon={<Zap color="#facc15"/>} trend="Model"/>
            </div>
            ...
            {!isAdmin ? (
              ...
              <div style={{...styles.mainCard, height:'400px', background: darkMode ? '#0f172a' : '#fff'}}>
                <h3>System-wide Inspection Performance</h3>
          ...
          /* HISTORY / LOGS TABLE */
          <div style={{...styles.mainCard, background: darkMode ? '#0f172a' : '#fff'}}>
            <div style={styles.cardHeader}><h3>{isAdmin ? "Global User Logs" : "My Scan History"}</h3></div>
...
// Reusable Components & Styles (Same as before but cleaned)
...
const StatBox = ({label, val, icon, trend}) => (
  <div style={{...styles.statBox, background:'#0f172a', border:'1px solid #1e293b'}}>
    <div style={styles.statIcon}>{icon}</div>
    <div><p style={{margin:0, color:'#64748b', fontSize:'0.75rem'}}>{label}</p><h2 style={{margin:0, fontSize:'1.2rem'}}>{val}</h2></div>
  </div>
);
...
const styles = {
  ...
  primaryBtn: { background: '#3b82f6', color: '#fff', border: 'none', padding: '15px 30px', borderRadius: '10px', display: 'flex', gap: '10px', fontWeight: 'bold' },
  ...
  mainHeader: { display: 'flex', justifyContent: 'space-between', marginBottom: '30px' },
  ...
  themeToggle: { background: '#0f172a', border: '1px solid #1e293b', color: '#fff', borderRadius: '10px', padding: '8px' },
  ...
  dropZone: { height: '250px', border: '2px dashed #1e293b', borderRadius: '15px', display: 'flex', justifyContent: 'center', alignItems: 'center', background: '#020617' },
  previewBox: { height: '100%', objectFit: 'contain' },
  ...
  statusChip: { display: 'inline-block', padding: '5px 15px', borderRadius: '20px', color: '#fff', fontWeight: 'bold' },
